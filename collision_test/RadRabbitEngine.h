...
namespace RRE {
	...
	public:
		...
		static bool IsIntersecting(const LineSegment& seg1, const LineSegment& seg2)
		{
			...
			//Can potentially further optimize this by taking x and y distances of each Line Segment at construction, but that likely won't help much
	...
	public:
		...
		Shape(pnt_F* vertices) { this->vertices = vertices; }
		~Shape() {
			delete[] vertices;
		}
		...
		static bool CheckCollision(const Shape sh1, const Shape sh2) {
			...
			for (int i = 0; i < sh1.vertexCount; i++) //Check collision using the first shape
			{
				...
				projection1 = Shape::TakeProjection(axis, sh1); // Project shape1 onto the axis
				projection2 = Shape::TakeProjection(axis, sh2); //Project shape2 onto the axis
		...
		static LineSegment TakeProjection(const Line line, const Shape shape)
		{

			//Project each point onto this line
			pnt_F* projectedPoints = new pnt_F[shape.vertexCount];
			float tempX;
			float slope = line.slope; //Let's get this now so we don't have to keep following the pointer

			for (int i = 0; i < shape.vertexCount; i++)
			{
				tempX = (shape.vertices[i].x + slope * shape.vertices[i].y - slope * line.yInt) / (slope*slope + 1); //Find the x coordinate of the projection
				projectedPoints[i] = *(new pnt_F(tempX, slope*tempX + line.yInt)); //Construct and dereference a point for this projected point
			}
		...
		void setVertex(pnt_F vertex, int vertexNumber = 0){ this->vertices[vertexNumber] = vertex;} //Sets the vertex at the given index to the point given
	};
